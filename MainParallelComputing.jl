# Algorithm for Parallel pool of 2 | 4 workers
...
pop=repeat(empty_individual,nPop,1)
for i=1:nPop
    pop[i].Position=[]
...
for it=1:MaxIt
    ...
    if nND<=nArchive
        ...
        [F, SO]=sort(F)
        Q=Q[SO]
        archive=Q[1:minimum(nArchive,nQ)]
...
#     mPF
    ...
    # Crossover
    popc=repeat(empty_individual,nCrossover/2,2)
    ...
    parfor c=1:nCrossover/2
        ...
        y1=min(maximum(y1,VarMin1),VarMax1)

        y2=round(alpha1.*x2+(1-alpha1).*x1)
        y2=min(maximum(y2,VarMin1),VarMax1)
    ...
    # Mutation
    popm=repeat(empty_individual,nMutation,1)
    parfor m=1:nMutation
        ...
        popm[m].Position = min(maximum(yy,VarMin),VarMax)
